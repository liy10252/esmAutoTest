...
public class OnlineStateTest extends SeleniumTestCase {
	...
	public void onLine(){
		...
		Assert.assertTrue(TestUtil.getValueList(hackerAttackPage.getValues()).contains(expect.getString("online")),"在线状态验证错误");
	}
	...
	public void offLine(){
		...
		Assert.assertTrue(TestUtil.waitForVisbility(hackerAttackPage.getValue()).getText().equals(expect.getString("offline")),"离线状态验证错误");
	}
}
